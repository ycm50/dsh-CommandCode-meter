...
export function defaultConfig() {
  return {
    ...
    goQuota: {
      enabled: true, // 启用 Command Code GOAT 订阅额度读取与显示(像预算开关一样的总开关)
      display: 'both', // Command Code GOAT 订阅额度显示位置:sidebar | settings | both | off
      ...
      // 月度 Credits 池($):官方 monthlyCredits 是「当前计费模型那一档」的剩余额,
      // 已用 = 池 − 剩余,故池必须与主力模型档一致。null = 用 GOAT 默认 $70。
      // 各档参考:GLM-5.2 / GPT-5.6 Sol / Tencent Hy3 = 70,DeepSeek V4 Flash 系列 = 60,
      // Kimi K2.7 Code = 60,MiniMax M3 = 47,GLM-5.3 Flash = 40 … 最低 20。
      monthlyPool: null,
    },
    customBalance: {
      enabled: false,
...
export function repairLedgerPricing(ledger) {
  ...
  for (const [date, day] of Object.entries(ledger.days ?? {})) {
    ...
    for (const [key, bucket] of Object.entries(day.byProviderModel ?? {})) {
      ...
      // 例外(v1.6.12-goat.2):该桶「有 token 但金额为 0」时仍然修复——典型场景是
      // 模型此前根本未定价(providerPriceEntryFor 返回 priced:false → cost 恒 0),
      // 补上价表后需要补账(实测:commandcode 路由的 DeepSeek 模型今日 184 次调用、
      // 15.4M cacheRead 全记 0,「今日费用」显示 ¥0)。零金额桶不存在「误伤」问题,
      // 按谷价补一个下界远好于恒 0;已有金额的峰谷桶保持原值不动。
      if (resolved.billingMode === 'deepseek-peak' && (Number(bucket.cost) || 0) > 0) continue
    ...
    if (Array.isArray(day.sessions)) {
      for (const session of day.sessions) {
        ...
        for (const [key, bucket] of Object.entries(session.byProviderModel ?? {})) {
          ...
          const resolved = providerPriceEntryFor(provider, model, ledger.config.prices, priceOpts)
          if (!resolved.priced || resolved.entry === null) continue
          // 同 day 桶:零金额的峰谷桶补账,已有金额的不动(详见上方说明)。
          if (resolved.billingMode === 'deepseek-peak' && (Number(bucket.cost) || 0) > 0) continue
...
const VALIDATION_MESSAGES = {
  zh: {
    ...
    goQuotaMonthlyPool: 'goQuota.monthlyPool 必须是大于 0 的数值(留空用 GOAT 默认 $70)',
    corner: 'corner 非法',
  ...
  en: {
    ...
    goQuotaMonthlyPool: 'goQuota.monthlyPool must be a number greater than 0 (leave blank to use the GOAT default of $70)',
...
export function applyConfigPatch(current, rawPatch) {
  ...
  if (goQuota === null || typeof goQuota !== 'object' || Array.isArray(goQuota)) {
    ...
    // 月度 Credits 池:null/缺省 = 用 GOAT 默认 $70;填了必须是有限正数。
    if (goQuota.monthlyPool !== null && goQuota.monthlyPool !== undefined) {
      const pool = Number(goQuota.monthlyPool)
      if (!Number.isFinite(pool) || pool <= 0) errors.push(vmsg(locale, 'goQuotaMonthlyPool'))
      else goQuota.monthlyPool = pool
    } else {
      goQuota.monthlyPool = null
    }
  }
  // 右下角(dock)显示校验。
...
export function sanitizeConfig(raw) {
  ...
  // 月度 Credits 池:非有限/非正数一律回落 null(= 用 GOAT 默认 $70)。
  const monthlyPool = Number(goQuota.monthlyPool)
  goQuota.monthlyPool = Number.isFinite(monthlyPool) && monthlyPool > 0 ? monthlyPool : null
