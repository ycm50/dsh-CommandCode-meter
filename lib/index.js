...
import { CODING_PLAN_PROVIDERS, CODING_PLAN_PROVIDER_IDS, queryCodingPlan, scnetTokenPlanWindows, emptyCustomBalance, queryCustomBalance, normalizeVolcengineKey, COMMAND_CODE_PLANS, DEFAULT_COMMAND_CODE_PLAN, GOAT_WINDOW_CAPS, normalizeMonthlyPool } from './coding-plans.js'
...
const SERVER_MESSAGES = {
  zh: {
    ...
    goQuotaNoUsage: 'Command Code 额度响应中未解析出任何用量窗口(缺少 windowLimits 与 monthlyCredits),接口结构可能已变化',
  ...
  en: {
    ...
    goQuotaNoUsage: 'The Command Code quota response contained no usable usage window (neither windowLimits nor monthlyCredits); the API shape may have changed',
...
/**
 * GOAT 月度 Credits 池默认值($10/月 → $70 等效用量)。
 *
 * 口径(2026-09 依官方文档复核,见 coding-plans.js 的 COMMAND_CODE_PLANS):
 *  - $70 是 GOAT 的**月度 Credits 总额**;官方模型表里每个模型另有各自的
 *    Monthly credits(GLM-5.2 / GPT-5.6 Sol / Tencent Hy3 = $70,DeepSeek V4 Flash
 *    系列 = $60,Kimi K2.7 Code = $60,GLM-5.3 Flash = $40 …低至 $20)。
 *  - 因此 /alpha/billing/credits 的 monthlyCredits 是**当前计费那一档**的剩余额,
 *    不是账户全局剩余;「已用 = 池 − 剩余」只有在池取成同一档的 allowance 时才成立。
 *  - 故此处保留为**可配置**缺省值:设置页「月度额度池」留空即用 $70,换套餐/换主力
 *    模型档位时填对应 allowance(如 DeepSeek V4 Flash 档填 60),不必改代码。
 *  - 5 小时 / 周 滚动窗上限($14 / $35)由官方接口直接返回 used/cap,不依赖本常量,
 *    仅作为文档与诊断参考(见 GOAT_WINDOW_CAPS)。
 */
const GOAT_MONTHLY_POOL = COMMAND_CODE_PLANS[DEFAULT_COMMAND_CODE_PLAN].monthlyCredits

/**
 * 解析本次应采用的月度 Credits 池($):
 * 配置 goQuota.monthlyPool(有限且 > 0)优先,否则回落 GOAT 默认 70。
 * @param config - 插件配置。
 * @returns {{ pool: number, source: 'config' | 'default' }}
 */
function resolveMonthlyPool(config) {
  const configured = normalizeMonthlyPool(config?.goQuota?.monthlyPool)
  return configured !== null ? { pool: configured, source: 'config' } : { pool: GOAT_MONTHLY_POOL, source: 'default' }
}
 ...
 * weekly: {...} } }(issue #30 实测):
 *  - rolling ← windowLimits.fiveHour(used/cap 已用%;官方上限 GOAT=$14),
 *  - weekly  ← windowLimits.weekly(官方上限 GOAT=$35),
 *  - monthly ← 月度 Credits 池:已用 = 池 − monthlyCredits,池取配置 goQuota.monthlyPool
 *    (缺省 $70 = GOAT 月度 Credits)。
 *
 * 口径关键(勿改回「全局 $70」):monthlyCredits 是**当前计费模型那一档**的剩余额,
 * 官方各模型的 Monthly credits 从 $20 到 $70 不等,所以池必须与当前档位一致——
 * 用 $70 去减一个 $40 档的模型会算出负值/虚高已用。故该值做成可配置项而不是常量。
 * 5h/周两档是固定的 credit 值上限,与混用哪些模型无关,直接采信接口的 cap。
 *
 * 映射到卡片三档:rolling ← fiveHour,weekly ← weekly,monthly ← 月度池已用%。
 * 凭证只发往官方域名 api.commandcode.ai;Key 解析顺序见 resolveGoKey。
 * @param ctx - 宿主插件上下文(用于解析 DSH 凭据库中的 Key)。
 * @param config - 插件配置(goQuota.apiKey / goQuota.monthlyPool / 消息语言)。
...
async function queryGoQuota(ctx, config, locale) {
  ...
  const rolling = windowOfCommandCode(limits?.fiveHour)
  const weekly = windowOfCommandCode(limits?.weekly)
  const { pool, source: poolSource } = resolveMonthlyPool(config)
  let monthly = null
  const monthlyCredits = Number(data?.credits?.monthlyCredits)
  if (Number.isFinite(monthlyCredits) && monthlyCredits >= 0) {
    // 剩余额超过所选池时(池填小了 / 官方给了更高的模型档)夹到 0 而不是出负数,
    // 并把剩余额与 poolTooSmall 标注出来,便于用户按提示改「月度额度池」配置。
    const remaining = Math.max(0, monthlyCredits)
    const used = Math.max(0, pool - remaining)
    monthly = {
      percent: Math.max(0, Math.min(100, (used / pool) * 100)),
      resetsAt: '',
      used,
      cap: pool,
      remaining,
      poolSource,
      // 剩余额大于池:池配置与当前模型档位不一致,面板据此提示核对。
      poolTooSmall: remaining > pool,
    }
  }
  // 三档全空:接口结构变化(或账户无套餐),报结构化错误而不是静默返回空面板。
  if (rolling === null && weekly === null && monthly === null) {
    throw new Error(tmsg(locale, 'goQuotaNoUsage'))
  }
