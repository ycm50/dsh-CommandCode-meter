...
/**
 * Command Code 订阅套餐表(官方定价/限额,2026-09 抓取复核)。
 *
 * 来源:
 *  - https://commandcode.ai/docs/plans/goat(GOAT 套餐:$10/月 → $70 等效用量,7× 倍数)
 *  - https://commandcode.ai/docs/resources/pricing-limits(全套餐对照表)
 *  - https://commandcode.ai/docs/resources/usage-limits(逐套餐 5 小时/周档上限)
 *
 * 每个套餐 = 月费 + 月度 Credits 池($ 等效用量)+ 两个滚动窗口上限(滚动 5 小时 / 周)。
 * 滚动窗口从「本窗首次请求」起算,不是固定时钟;窗口只对**套餐内月度 Credits** 生效,
 * 按量付费(on-demand / 额外 Credits)不计入也不受限。
 *
 * 重要口径(官方 Usage Limits 文档):
 *  - 多数套餐 1 credit = $1 用量;但 GOAT / Pro 的 credit 是「用量价值单位」——
 *    满额模型(GLM-5.2、GPT-5.6 Sol、Tencent Hy3)1 credit = $1 用量,低额模型
 *    每个 credit 折抵更多用量。5h/周上限是按 credit 值计的固定额度,**与混用哪些
 *    模型无关**:GOAT 任意 5 小时可用 14 credits、任意 7 天可用 35 credits。
 *  - 月度池($70)是「按模型分别给的 allowance 之和」的口径:官方模型表里每个模型
 *    各有自己的 Monthly credits($70/$60/$47/$40/$33/$30/$20)。因此
 *    `/alpha/billing/credits` 返回的 monthlyCredits 指的是**当前计费模型那一档**
 *    的剩余额,不等于「账户全局剩余 $70」;月度已用需按同一档的 allowance 反推。
 */
export const COMMAND_CODE_PLANS = {
  go: { label: 'Go', price: 1, monthlyCredits: 10, fiveHourCap: 3, weeklyCap: 6 },
  goat: { label: 'GOAT', price: 10, monthlyCredits: 70, fiveHourCap: 14, weeklyCap: 35 },
  pro: { label: 'Pro', price: 20, monthlyCredits: 80, fiveHourCap: 16, weeklyCap: 40 },
  max10x: { label: 'Max 10×', price: 100, monthlyCredits: 150, fiveHourCap: 45, weeklyCap: 90 },
  max20x: { label: 'Max 20×', price: 200, monthlyCredits: 300, fiveHourCap: 90, weeklyCap: 180 },
  teamPro: { label: 'Team Pro', price: 40, monthlyCredits: 40, fiveHourCap: 12, weeklyCap: 24 },
}

export const COMMAND_CODE_PLAN_IDS = Object.keys(COMMAND_CODE_PLANS)

/** 默认套餐(本 fork 面向 GOAT 订阅):$10/月 → $70 月度 Credits,5h=$14 / 周=$35。 */
export const DEFAULT_COMMAND_CODE_PLAN = 'goat'

/**
 * GOAT 套餐各窗口官方上限(GOAT_MONTHLY_POOL 缺省值 + 滚动窗上限)。
 * 供 index.js 在配置未覆盖时兜底;换套餐时可据此改配置而非改常量。
 */
export const GOAT_WINDOW_CAPS = {
  monthly: COMMAND_CODE_PLANS.goat.monthlyCredits,
  fiveHour: COMMAND_CODE_PLANS.goat.fiveHourCap,
  weekly: COMMAND_CODE_PLANS.goat.weeklyCap,
}

/** 归一化月额度池配置值:有限且 > 0 才接受,否则返回 null(交由调用方兜底)。 */
export function normalizeMonthlyPool(value) {
 ...
 *
 * 口径提醒(见 COMMAND_CODE_PLANS 注释):monthlyCredits 是**当前计费模型那一档**的
 * 剩余额,不是账户全局剩余;跨模型求和会凭空造出一个不存在的桶,故此处只作为
 * 原始余额透传,不做「池 − 剩余」的月度已用推导(该推导需要知道当前档位的 allowance,
 * 由宿主侧 queryGoQuota 按可配置的月度池完成)。
 */
