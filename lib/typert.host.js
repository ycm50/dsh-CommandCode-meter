 ...
 * `./typert` 导出 TYPERT,invocations 的 codec 必须是
 * `{ mode: 'strict', typeSymbol, schema: zodSchema, create: () => zodSchema }` 形状:
 * `schema` 供 0.1.5-rc.x 的 typert-loader(`requireStrictCodec()` 要求 zod v4 实例)
 * 与那代客户端 registry(`codec.schema.parse`)使用;`create` 为惰性工厂,供 0.1.7+
 * 的 loader / 客户端 registry(`codec.create()` 强制)使用。两代宿主的键名不同,
 * 故两个键都写才能通用(缺任一键,对应那代宿主会整份拒收本包的 typert 贡献)。
 */
...
const configSchema = z.object({
  ...
  goQuota: z.object({
    ...
    detail: z.boolean(),
    // 月度 Credits 池($):null = 用 GOAT 默认 $70(见 coding-plans.js 的 COMMAND_CODE_PLANS)。
    monthlyPool: z.union([num, z.null()]).optional(),
...
const goWindowSchema = z.union([
  z.object({
    percent: num,
    resetsAt: z.string(),
    // v1.6.13:附带具体额度(已用/总量),供客户端显示「已用$/总量$ 百分比%」。
    used: num.optional(),
    cap: num.optional(),
    // 月度 Credits 池专用:剩余额与池来源(config/default);remaining > cap 时
    // poolTooSmall=true,提示「月度额度池」配置与当前模型档位不一致。
    remaining: num.optional(),
    poolSource: z.enum(['config', 'default']).optional(),
    poolTooSmall: z.boolean().optional(),
  }),
  z.null(),
])
...
const codingPlanSchema = z.object({
  ...
  message: z.string(),
  fetchedAt: num,
  windows: z.record(z.string(), z.object({ percent: num.optional(), resetsAt: z.string(), text: z.string().optional(), used: num.optional(), cap: num.optional() })),
})
...
const _state$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#CostState', schema: stateSchema, create: () => stateSchema }
const _patch$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#ConfigPatch', schema: patchSchema, create: () => patchSchema }
const _fetch$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#FetchPricesResult', schema: fetchPricesSchema, create: () => fetchPricesSchema }
const _provider$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#CodingPlanProvider', schema: z.string(), create: () => z.string() }
// 密钥目标(v1.6.8):'goQuota' | 'codingPlans.<id>' | 'codingPlans.volcengine.ak' | '...sk'
const _credTarget$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#CredentialTarget', schema: z.string(), create: () => z.string() }
// 密钥明文(v1.6.8):仅经 setCredential 单向写入凭据库,永不回传前端。
const _credValue$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#CredentialValue', schema: z.string(), create: () => z.string() }
const _day$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#DayRecord', schema: daySchema, create: () => daySchema }
const _date$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#DayKey', schema: z.string(), create: () => z.string() }
const _topSessions$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#TopSessions', schema: topSessionsSchema, create: () => topSessionsSchema }
const _limit$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#SessionLimit', schema: num, create: () => num }
const _sort$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#SessionSort', schema: z.string(), create: () => z.string() }
const _dir$codec = { mode: 'strict', typeSymbol: 'dsh-CommandCode-meter#SessionSortDir', schema: z.string(), create: () => z.string() }
